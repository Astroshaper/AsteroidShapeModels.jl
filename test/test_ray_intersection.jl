...
@testset "Ray intersection tests" begin
    ...
    @testset "BVH Pre-build Requirement" begin
        # Test that BVH must be pre-built for ray intersection
        ...
        # Ray intersection should throw error when BVH is not built
        ray = Ray(@SVector([0.25, 0.25, 1.0]), @SVector([0.0, 0.0, -1.0]))
        @test_throws ArgumentError intersect_ray_shape(ray, shape_no_bvh)
        
        # Build BVH manually
        build_bvh!(shape_no_bvh)
        @test !isnothing(shape_no_bvh.bvh)
